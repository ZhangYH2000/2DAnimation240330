...
tile::tile(FILE* f) {
	frd(w); frd(h);
	cols.resize(w * h); as.resize(w * h);
	frds(&cols[0], n()); frds(&as[0], n());
}
void tile::save(FILE* f) const {
	fwt(w); fwt(h);
	fwts(&cols[0], n()); fwts(&as[0], n());
}
...
tile flip_x(tile const& t) {
	tile out(t.w, t.h);
	rep(y, 0, t.h) rep(x, 0, t.w) {
		int dp = y * t.w + x;
		int sp = y * t.w + t.w - 1 - x;

		out.as[dp] = t.as[sp];
		out.cols[dp] = t.cols[sp];		
	} return out;
}
