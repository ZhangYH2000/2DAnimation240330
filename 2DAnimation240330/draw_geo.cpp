...
void draw_eclipse(tile& dest, vec2 ct,
	double ax, double ay, double ang, drect vp, dcol col) {
	mat2 inv_tsf = mat2::rot(-ang);
	vec2 hwh_tsf = inv_tsf.abs() * vec2(ax, ay);
	// 这个 1 是经验公式。我没做严格论证。
	int a = max(vp.left(), int(ct.x - hwh_tsf.x) - 1);
	int b = min(vp.right(), int(ct.x + hwh_tsf.x) + 1);
	int c = max(vp.top(), int(ct.y - hwh_tsf.y) - 1);
	...
	rep(i, a, b) rep(j, c, d) {
		...
		// 下面式子的正确性从逆变换比较容易看，要注意四则运算的优先级。
		vec2 u_pnt = 
			inv_tsf * (vec2(d_pnt) - ct) / vec2(ax, ay);
		if (u_pnt.lensqr() < 1) {
			int dp = d_pnt.y * dest.w + d_pnt.x;
...
void draw_eclipse_frame(tile& dest, vec2 ct,
	double ax, double ay, double ang, drect vp, double thk, dcol col) {
	mat2 tsf = mat2::rot(ang);
	
	int n = 40;
	rep(i, 0, n) {
		double a0 = 2 * PI * double(i) / n;
		double a1 = 2 * PI * double(i + 1) / n;
		vec2 v0 = ct + tsf * vec2(ax * cos(a0), ay * sin(a0));
		vec2 v1 = ct + tsf * vec2(ax * cos(a1), ay * sin(a1));
		draw_tiny_seg(dest, v0, v1, thk, vp, col);
	}
}
void draw_px_eclipse_frame(tile& dest, vec2 ct,
	double ax, double ay, double ang, drect vp, dcol col) {
	mat2 tsf = mat2::rot(ang);
